...
export let messageDataSourceNew: { [key: string]: Object }[] = [
  ...
  {
    "ContactID": "SF10182",
    "ContactName": "Anastasia Callahan",
  ...
  {
    "ContactID": "SF10181",
    "ContactName": "Prescott Mccormick",
  ...
  {
    "ContactID": "SF10066",
    "ContactName": "Wyoming Cunningham",
    "ContactTitle": "Invitation for farewell party of Mr",
...
export function getContacts(): { [key: string]: Object }[] {
    ...
    for (let i: number = 0; i < messageDataSourceNew.length; i++) {
        addContacts(messageDataSourceNew[i], 'Email', 'ContactName', contacts1);
    }
    return contacts1;
}

function addContacts(messageData: { [key: string]: Object }, mailId: string, contactName: string, contacts: { [key: string]: Object }[]): { [key: string]: Object }[] {
    ...
    if (messageData[mailId]) {
        if (messageData[mailId] instanceof Array) {
            ...
            let contactsList: string[] = messageData[contactName] as string[];
            for (let j: number = 0; j < mailIdList.length; j++) {
                contactData = {};
                if (!isContactNameExist(contacts, mailIdList[j])) {
                    fieldId = 'MailId';
            ...
            if (!isContactNameExist(contacts, messageData[mailId].toString())) {
                contactData[fieldId] = messageData[mailId];
                mailId = 'ContactName';
                contactData[mailId] = messageData[contactName];
                contactData['Image'] = messageData['Image'];
                contacts.push(contactData);
            }
        }
...
function isContactNameExist(contacts: { [key: string]: Object }[], contactName: string): boolean {
    let key: string = 'MailId';
    for (let i: number = 0; i < contacts.length; i++) {
        if (contacts[i][key]) {
            if (contacts[i][key].toString() === contactName) {
                return true;
            }
        }
    }
